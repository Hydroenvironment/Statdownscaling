...
setwd('D:/charles/regionalizacion/')
...
fecha_in_his_min <- as.Date('1950-01-01')
fecha_fin_his_min <- as.Date('2019-12-31')

fecha_in_his_max <- as.Date('1950-01-01')
fecha_fin_his_max <- as.Date('2019-12-31')

fecha_in_his_pr <- as.Date('1950-01-01')
...
ds <- function(time_ini_his, time_fin_his, time_ini, time_fin, data_his, data_model, data_model_his, var){
  ...
  for (j in 1:n_model) {
    ...
    seasons_by_year <- list(c("Diciembre"), c("Enero"), c("Febrero"),
                            ...
                            c("Setiembre"), c("Octubre"), c("Noviembre"))
...
for (z in 1:3) {
  ...
  write.csv2(list_time, file = name_file_out,row.names = F)
